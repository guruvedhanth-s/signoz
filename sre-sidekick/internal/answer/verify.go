...
//
// It extracts every numeric token from the candidate answer and requires
// each one to appear in the set of values the composer supplied. A token
// that was never supplied means the model produced a figure from nowhere,
// which is a bug, not a stylistic choice - the caller must discard the
// answer and fall back to the deterministic template.
//
...
// is built from exactly what the composer handed the model - values,
// labels, window, evaluated range, service and environment - so the
// allowlist and the prompt cannot drift apart: both are derived from the
// same Facts.
